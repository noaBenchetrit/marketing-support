...
const DemoFormSchema = z.object({
  ...
  centre: z.string().trim().min(2, 'Nom de votre entreprise requis').max(120),
...
export async function submitDemo(formData: FormData): Promise<DemoFormResult> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[formulaire/submitDemo] CRM a retourné un statut non-2xx', {
    ...
    console.log('[formulaire/submitDemo] Lead envoyé au CRM', {
    ...
    console.error('[formulaire/submitDemo] Échec de l\'appel CRM', {
